...
export async function POST(req: NextRequest) {
  ...
  if (model) overrides[bot] = model;
  else delete overrides[bot]; // empty clears the override (back to env default)

  try {
    const { error } = await getSupabase()
      .from("bot_config")
      .upsert({ key: "model_overrides", value: overrides, updated_at: new Date().toISOString() }, { onConflict: "key" });
